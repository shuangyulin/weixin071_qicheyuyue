				...
				<swiper class="swiper" :indicator-dots='".swiper-pagination"==null?false:true' :autoplay='autoplaySwiper' :circular='true' indicator-active-color='rgba(45, 234, 234, 1)' indicator-color='rgba(0, 0, 0, .3)' :duration='1000' :interval='intervalSwiper' :vertical='"horizontal"=="horizontal"?false:true'>
					<swiper-item v-for="(swiper,index) in swiperList" :key="index" @tap="onSwiperTap(swiper)">
						<image mode="aspectFill" :src="swiper.img"></image>
														...
														<!--header end-->
		<view class="pl15 col3" style="padding: 0 12px;box-sizing: border-box;" :style='{"backgroundColor":"rgba(74, 209, 242, 1)","borderColor":"rgba(132, 122, 122, 1)","color":"rgba(255, 255, 255, 0.64)","borderRadius":0,"textAlign":"center","borderWidth":"2rpx","fontSize":"32rpx","lineHeight":"72rpx","borderStyle":"solid"}'>技师信息推荐</view>
		<view v-if='2 == 1 || 2 == 2' class="uni-product-list" :class="2 == 2 ? 'active' : ''" :style='{"borderRadius":0,"backgroundColor":"rgba(135, 128, 128, 0.34)"}'>
			<view @tap="onDetailTap('jishixinxi',product.id)" :style='{"borderRadius":"12rpx","backgroundColor":"#fff"}' class="uni-product" v-for="(product,index) in jishixinxilist" :key="index">
				                				                				<view class="uni-product-title" :style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"#333","textAlign":"center"}'>{{product.jishixingming}}</view>
								                				                				                				                				                				<view class="image-view" :style='{"borderRadius":"20rpx","height":"320rpx"}'>
					<image mode="aspectFill" :style='{"borderRadius":"20rpx","height":"320rpx"}' class="uni-product-image" :src="product.zhaopian?product.zhaopian.split(',')[0]:''"></image>
				</view>
                				                				                				                				                				                				                				                				                							</view>
		</view>
		<view class="list" v-if='2 == 3' :style='{"borderRadius":0,"backgroundColor":"rgba(135, 128, 128, 0.34)"}'>
			<view :style='{"borderRadius":"12rpx","backgroundColor":"#fff"}' @tap="onDetailTap('jishixinxi',product.id)" v-for="(product,index) in jishixinxilist" :key="index" class="listm flex flex-between">
																																				<view class="listmr">
					<view class="col3 f30 elip mb15" :style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"#333","textAlign":"center"}'>{{product.jishixingming}}</view>
				</view>
																																																																																																								<image mode="aspectFill" :style='{"borderRadius":"20rpx","height":"320rpx"}' class="listmpic" :src="product.zhaopian?product.zhaopian.split(',')[0]:''"></image>
																																																																																																																																																																																							</view>
		</view>
		<!--list end-->
	...
	export default {
		...
		async onShow() {
			...
			this.news = res.data.list
																					...
																					// 推荐信息
			res = await this.$api.recommend('jishixinxi', 1, 6);
			this.jishixinxilist = res.data.list
